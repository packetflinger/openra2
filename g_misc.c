 ...
 * separated when this portal is not activated. Usually enclosed in the middle
 * of a door.
 */
...
/**
 * Create a new gib edict and let it fly
 */
...
void SP_path_corner(edict_t *self) {
    if (!self->targetname) {
        gi.cprintf(NULL, PRINT_HIGH, "path_corner with no targetname at %s\n", vtos(self->s.origin));
        G_FreeEdict(self);
        return;
    }
...
void SP_viewthing(edict_t *ent) {
    gi.cprintf(NULL, PRINT_HIGH, "viewthing spawned\n");
...
void SP_func_wall(edict_t *self) {
    ...
    if (self->spawnflags & 4) {
        if (!(self->spawnflags & 2)) {
            gi.cprintf(NULL, PRINT_HIGH, "func_wall START_ON without TOGGLE\n");
            self->spawnflags |= 2;
        }
    }
...
/**
 * Not really a monster, this is the Tank Commander's decapitated body. There
 * should be a item_commander_head that has this as it's target.
 */
...
/**
 * This is a Strogg ship for the flybys. It is trigger_spawned, so you must
 * have something use it for it to show up. There must be a path for it to
 * follow once it is activated.
 *
...
/**
 * don't let field width of any clock messages change, or it could cause an
 * overwrite after a game load
 */
