...
/**
 * Cycle through items, selecting the next available one
 */
...
/**
 * Cycle through items in reverse, selecting the next available one
 */
...
/**
 * Ensure the selected item is actually ent's inventory
 */
...
/**
 * Display if cheats are disabled or not
 */
...
/**
 * List all arenas and select one if arg is provided.
 */
...
/**
 * Show the menu (for backwards compatibility)
 */
...
/**
 * Return whether ent should be able to do something or not based on
 * excessive amounts of that same action previously
 */
...
static size_t build_chat(edict_t *ent, chat_t chat, int start, char *buffer) {
    ...
    int i;
    char *p;
    int arena = ARENA(ent)->number;

    switch (chat) {
        case CHAT_TEAM:
            total = Q_scnprintf(buffer, MAX_CHAT, "(%s): ", NAME(ent));
            break;
        case CHAT_ALL:
            total = Q_scnprintf(buffer, MAX_CHAT, "%s (arena %d): ", NAME(ent), arena);
            break;
        default:
            total = Q_scnprintf(buffer, MAX_CHAT, "%s: ", NAME(ent));
    }
...
/**
 * List the currently connected players in all areans
 */
...
/**
 * If ent is on a team, remove them and make them a spectator
 */
static void Cmd_Observe_f(edict_t *ent) {
    G_TeamPart(ent, false);
    G_SpectatorsJoin(ent);

    // TODO: "pregame" doesn't really exist, remove this
...
/**
 * Start chasing someone. Possible arg:
 *  "leader" or "top" for the high scorer
 */
...
/**
 * For testing sounds. Specify arg as relative path including extension:
 *   "world/10_0.wav"
 *
 * TODO: remove later
 */
...
static void Cmd_Admin_f(edict_t *ent) {
    ...
    G_CheckVote(); // admin's vote wins automatically
    G_CheckArenaVote(ARENA(ent)); // ditto
}
 ...
 * ban arg will cause this player's IP to be added to the filter list
 */
...
/**
 * Player chose an arena from the menu, change to it
 */
...
/**
 * Player chose a team to join from the menu
 */
...
/**
 * The main menu when you connect
 */
...
/**
 * Show the menu allowing for arena selection
 */
...
/**
 * Display the scoreboard and then playerboard
 */
...
/**
 * Toggle the lock on a team to prevent players from joining or not
 */
